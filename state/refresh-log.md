...
## 2026-07-08 — MILESTONE run (first paying customer)
- **FIRST PAYING CUSTOMER (Jul 2):** organic/direct, Growth ANNUAL, converted in-app a day after the aha via the try-on pricing prompt. NOT email (click 0), NOT paid (checkouts concentrate Direct 11 vs Meta 2). The queued email/product fixes were NOT the lever. Goal hit, ~1 week late.
- **3 due predictions all MOOT** (try-generate, onboarding-filter, cta-rework -- none shipped). LES written incl. the first-customer profile and the amount_usd=0 revenue bug.
- **Re-ranked for repeatability:** #1 convert the 11 warm checkout-starters -> customer #2; #2 fix revenue tracking (amount_usd=0, server-side); #3 at-aha price-ask (VALIDATED by the payer, spec staged); #4 reverse activation regression (51->46); #5 shift budget to Swim (now the efficient segment, $7.27 vs Kids $34.73, 61% budget mis-allocated); #6 Meta feed frozen again since 07-03.
- **Staged the validated lever:** at-aha-price-ask spec (copy + placement + instrumentation) so the proven conversion path is build-ready, not just re-listed.
- **Proposes next:** Jason: fix amount_usd + server-side purchase + Subscription Started; build the at-aha price-ask (spec ready). Alejo: reallocate budget to Swim; verify the autopilot cron (Meta feed frozen). The product converts organic high-intent users at the aha, feed and sharpen that path.

## 2026-07-08 (QA + publish)
- **QA (2 skeptics): numbers=revise, calls+boundary=pass.** Boundary clean (only reversible files + 2 staged drafts; no flow/budget/product/cron change). Moot calls and the n=1 re-rank both UPHELD as grounded in weeks of channel data. Fixes applied: softened CC #5 Swim CPLs to qualitative (4th unpersisted-adset repeat), tied the milestone claim to channel data not the lone payer, flagged CC #1 as a one-shot cohort. The skeptic's "moot=3" was a stale-data.js snapshot (pre-rebuild); the ledger is 6 moot / 5 miss / 1 hit.
- **Scoreboard honesty:** calibration 0.17 is 1 hit / 6 SCORED; the other 6 resolved are moot (unshipped/unexecuted), so tested-bet skill is 1/6 and the real signal is the execution gap, not the diagnosis. 4 qa-lessons written (2 promote-to-skill: unpersisted-adset build guard, report-tested-rate).
