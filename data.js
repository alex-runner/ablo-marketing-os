...
window.ABLO_OS = {
  "meta": {
    ...
    "updatedISO": "2026-06-10T16:35:12.051109+00:00"
  },
  "overview": {
    ...
    "currentFocus": [
      "Fix the signup-modal drop. Only 126 of 292 opens finish (~43%). Google is the primary one-click CTA (since May 27) and now leads recent signups. Email is the demoted secondary option, yet still the top method overall (49 vs Google's 67). Keep it; don't drop email. The real leak is modal completion, not the auth method.",
      "Close the try-on activation gap. 141 entered the studio, 71 generated a model (50% never do). Find the broken or slow control before the first generate. Free-tier credit rebalance shipped to give new users two full generations.",
    ...
    "liveFallback": [
      {
        ...
        "signal": "Ended Jun 9 in PostHog (status: Complete per data.js live.experiments). Exposures below the ~200/variant power floor; no winner called. Flag now serving test variant at 100% rollout. With current traffic, prefer ship-and-measure before/after over PostHog A/B splits.",
        "id": "PH-374260"
      }
    ],
    "backlog": [
  ...
  "commandCenter": {
    ...
    "items": [
      ...
      },
      {
        ...
        "owner": "Alejo + Jason",
        "status": "10 lifetime checkout starters, 0 purchases visible. purchase_completed: 0 users in 30d, unverified. Klaviyo Convert flow live with no paid-exit event.",
      ...
      },
      {
        ...
        "owner": "Alejo",
        "status": "BELOW TARGET. Signup→tryon (full activation) 43% (54/125 spine). Signup→model 57% spine (71/125), down from 69% Jun-3; paid-mix dilution confirmed. Rage-clicks on /studio easing: 7-8/day Jun 6-7, down to 1/day Jun 9-10. Cohorted last-7d model rate ~40-45%. Coachmarks PH-374260 concluded Jun 9 inconclusive, flag at 100%. PRED-2026-06-03-activate-flow due Jun 24.",
        "targets": "activation_rate · target 50%+",
        "body": "Before treating the slide as a product regression, split last-7d activation by UTM channel: if Direct also fell it is a product issue, otherwise it is channel mix and the fix is paid-audience quality, not onboarding. Adjacent evidence for the mix hypothesis (different metric and window, suggestive only): lifetime signup→try-on is 32% for the Meta cohort vs 57% for Direct. Coachmarks (PH-374260) ended in PostHog Jun 9, inconclusive (32/41 exposures, live read, not yet persisted), flag serving the variant at 100%. Activate flow live since May 19. PRED-2026-06-03-activate-flow due Jun 24.",
      ...
      },
      {
        ...
        "owner": "Alejo",
        "status": "Flight CPL $8.61 (24 signups, $206.71), under target. Last-24h CPL ~$23.6 (Δspend $23.64 on 1 signup), weekly trend rising 4.74→8.61.",
        "targets": "CPL < $20 · delivery on converting segments",
        "body": "Spend is concentrated on Kids+Swim by design: the size-inclusive and menswear ad sets were deliberately paused Jun 2 to keep the funded sets above Meta's learning floor. The Kids /toddler pixel under-fires (qualitative per data.js; ad-set-level counts are not persisted, so treat live-read figures as unaudited), so Meta optimizes partially blind on the top segment. Quality flag: paid drives 45% of signups but 1 of 10 checkouts; Direct drives 9.",
      ...
      },
      {
        ...
        "owner": "Alejo + Product",
        "status": "61% of /try paid visitors bounce before selecting a category (Jun-10 live: 107 of 272 progressed, 39%). Downstream holds (~60% wall→signup).",
        "targets": "/try view→signup (tbs cohort)",
        "body": "Since value-first launched Jun 4, 272 people viewed the /try page and only 107 (39%) picked a category. That is the single largest drop in the now-default paid funnel. After category selection the funnel holds (85 added a garment, 48 generated, 47 hit the signup wall, ~28 signed = ~60% wall to signup). The entire loss is at step one. Fix: larger tap targets, a one-line descriptor per category, or a pre-selected default. A 15pp category-selection lift translates to roughly 10 more signups per 272 visitors at current downstream rates.",
      ...
      },
      {
        ...
        "owner": "Alejo",
        "status": "Improving. Modal completion 43.5% last-7d (back from the 23% Jun-4 low). magic_link_requested at 0/day since removal.",
        "targets": "signup_completion_rate",
        "body": "Google-primary + magic-link removal are holding (+10pts vs the 33% baseline). Watch only, no new test on this surface until PRED-2026-06-04-remove-magic-link scores on Jun 18.",
  ...
  "live": {
    ...
    "experiments": [
      ...
      },
      {
        ...
        "started": "May 27, 2026",
        "hypothesis": "Promoting Continue with Google to the primary action and dropping email/password (magic link kept as a secondary link) lifts signup-modal completion. The magic-link path forced an inbox round-trip most people never returned from. Shipped May 27 (Google added), May 28 (email/password removed).",
        "signal": "Modal completion before May 27: 33% (22/66). After: 44% (96/220). Change: +10 pts.",
        "delta": 10.3,
        "before": 33.3,
        "after": 43.6,
        "shipped": true
      },
      {
        ...
        "started": "in progress (ClickUp 86ba9n6my)",
        "hypothesis": "Letting paid visitors generate a shot BEFORE signup (/try, value-first) should lift visitor→signup vs the homepage's signup-first wall. Control = homepage URL, variant = /try, identical ad set otherwise. This is a Meta-level landing split, so it has no PostHog experiment object; the OS measures it live from per-landing-page signup rate plus the /try value-first (tbs_*) funnel.",
        "signal": "Homepage 9.0% signup (60/670) vs /try 10.5% (30/286, tbs_* cohort). /try value-first funnel: 286 land -> 50 generate -> 49 hit the signup wall -> 30 signup. Note: /try is measured by its tbs_* cohort (true entry signal), correcting the earlier first-pageview-pathname undercount (the old 1.5% was an attribution artifact). Signups remain a floor: anonymous->identified magic-link stitching can split a signer off the cohort (ClickUp 86ba2wp4t). Thin sample, directional only.",
        "concluded": true,
        "conclusion": "Concluded 2026-06-08: /try 10% vs homepage 8.2% signup, /try-kids 10% vs /toddler 2.9%. Directional (thin samples) but consistent across every comparison. Decision: stop waiting for significance at this traffic level, make the value-first pages the default for all paid, repoint kids off /toddler. A/B retired so attention moves down-funnel to the binding constraint (signup->paid, 0 paying customers), not more top-of-funnel optimization.",
        "tryFunnel": {
          "landed": 286,
    ...
    "meta": {
      ...
      "asOf": "Jun 10, 2026",
      "deliveryFlag": "",
      "funnelHeadline": "Funnel stays healthy (CTR 3.5%, signup 9.5%, activation 64%, all above benchmark) and the top lever is unchanged: the 'try before signup' page still loses ~61% before anyone picks a category, the single largest drop in the funnel.",
      "funnelSuggestions": [
        {
          "step": "Tried the product",
          "severity": "high",
          "title": "The 'try before signup' page loses ~61% before anyone picks a category",
          "evidence": "Since this flow launched Jun 4: 272 people viewed the tbs page, only 107 selected a category (39%), the single largest drop in the whole funnel (~165 leave before doing anything). After that it holds: 85 added a garment, 48 clicked generate, 47 hit the signup wall. The leak is entirely at step one."
        },
        {
          "step": "Tried the product",
          "severity": "high",
          "title": "Rage-clicks keep landing on /studio right before the aha moment",
          "evidence": "30 people rage-clicked 62 times on /studio, ~25% of the 118 who entered the studio, and by far the most-rageclicked surface (next is /pricing at 3). It is easing (Jun 6-7 ran 7-8/day, Jun 9-10 down to 1/day) but has recurred almost daily for three weeks, so it is not a launch-week artifact."
        },
        {
          "step": "Signed up",
          "severity": "medium",
          "title": "Signup-modal leak is mostly healed; the remaining gap is desktop",
          "evidence": "Lifetime modal completion is 38.5% (104 of 270), but the round-trip leak is plugged: magic_link_requested has been 0 for 6 straight days (Jun 5-10), down from 14/day at launch, and only 8 of 67 requesters ever completed login (12%). The lagging segment is now desktop at 32.5% (27 of 83) vs mobile 41% (77 of 186)."
        },
        {
          "step": "Tried the product",
          "severity": "low",
          "title": "URL-import failures are historical, not a live wall, deprioritise",
          "evidence": "The 14-of-21 lifetime failure rate is dominated by the May-20 launch-day spike (24 failures that day alone). Recent days are quiet: Jun 8 = 0, Jun 9 = 1, Jun 10 = 0, and product_imported reached 39 people. Live volume is too thin to treat as a current bottleneck."
        }
      ]
    },
    "funnel": {
    ...
    "channels": {
      ...
      "insight": "52% of signups come from Direct / untagged — acquisition is dominated by untagged / organic traffic, not paid. Tag founder posts and referral links with UTMs to see what is really working, and weigh whether paid is earning its share.",
      "updated": "June 10, 2026",
    ...
    "landingPages": {
      ...
      "insight": "Homepage takes 670 visitors but only 24.8% click any CTA and 9.0% sign up. /toddler converts 3.9% to signup vs /try at 10.5%, so the landing page, not the ad, is the leak — a clean CRO test.",
      "window": "60d",
      "tryFunnel": {
        "landed": 286,
        "generated": 50,
    ...
    "learning": {
      "lessons": [
        ...
        {
          "type": "lesson",
          "id": "LES-2026-06-10-qa-live-vs-curated-funnel",
          "date": "2026-06-10",
          "lesson": "Always read data.js .live.funnel (live HogQL, updated daily) not .funnelCurated (cached snapshot, updated Jun 3): the cached section showed 64 signups/30 tryons/8 checkouts while the live section showed 125/54/10; nearly overwrote correct commandCenter status text with wrong numbers.",
          "evidence": "QA catch Jun 10: live.funnel.stages checkout all=10, tryon all=54, signup all=125 vs funnelCurated 8/30/64. The commandCenter items correctly use live figures. Edits to rank-2 and rank-3 were held after QA.",
          "confidence": "high",
          "tags": [
            "qa",
            "process",
        ...
        {
          "type": "lesson",
          "id": "LES-2026-06-05-qa-modal-peak",
          "date": "2026-06-05",
          "lesson": "Modal completion peaks in the first 10-day window post-ship then normalizes to a lower steady state: the Google-primary lift scored +11pp (33→44%) at day 10 but the all-window rate settled at +4pp (33→37%). Score conversion-fix predictions at 30-day steady state, not the peak window.",
          "evidence": "PRED-2026-05-20-google-signup resolved hit at 44% (10-day window); data.js 2026-06-05 OS-SIGNUP-GOOGLE after=37.2% (55/148 all post-May-27). QA catch: the CC rank-9 done item claimed '42%, held' but the live experiments signal shows 37%.",
          "confidence": "med",
          "tags": [
            "qa",
            "signup",
      ...
      "blindSpots": [
        {
          "key": "tbs_*",
          "dimension": "events",
          "where": "PostHog, 286 users/30d (13 events)",
          "volume": 286,
          "cluster": true,
        ...
        },
        {
          "key": "onboarding_*",
          "dimension": "events",
          "where": "PostHog, 52 users/30d (4 events)",
          "volume": 52,
          "cluster": true,
        ...
        },
        {
          "key": "seed_*",
          "dimension": "events",
          "where": "PostHog, 30 users/30d (1 events)",
          "volume": 30,
          "cluster": true,
