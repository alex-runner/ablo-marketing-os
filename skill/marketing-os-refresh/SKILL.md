...
- Homepage / landing-page positioning + CRO → `mkt1-homepage-positioning` (who / what / why-better / comparator, product type). Load it whenever a top-of-funnel bounce or a page-level bet is in play.
...
- **Top-of-funnel bounce is a first-class leak, never "normal".** The land→engage and land→signup steps lose the most humans by raw volume (≈78% leave before one click). Read the agent's `landing_pages` block (per-entry-page engage% + signup%, from `live.landingPages`). If the homepage or any paid landing page is the biggest leak by volume, it must carry a Command Center item; do not file it as expected homepage behavior, that blind spot is exactly what this routine was missing. A 2x signup-rate gap between two landing pages (e.g. /toddler vs /plus-size) is proof the page, not the ad, is the lever.

### 2b. Question the frame (acquisition + page bets, not just step fixes)
Steps tell you which *stage* leaks; this step asks whether the *page or the traffic* is the problem, the question a marketer asks that a step-by-step triage misses. Optimizing inside the funnel you were handed is not the same as asking if it is the right funnel. Spend one pass here every run:
- **Run the positioning lens on the worst high-volume entry page.** Load `mkt1-homepage-positioning` and judge the homepage (and the worst-converting paid landing page in `landing_pages`) on who / what / why-better / comparator, product type = vertical solution for fashion brands. A failing answer is a CRO hypothesis, not a vibe.
- **Propose at least one page or acquisition bet, not only a step fix.** A sharper hero, a landing-page variant matched to the ad, a distinct entry page per segment, or a traffic-source shift. Ladder it to land→signup and the goal, log it as a `prediction`, and queue it on the experiment roadmap (step 7). **UTM-tag every variant** so `live.landingPages` and the channels block can read the lift; an untracked page test is not a test.
- **Never let a big top-of-funnel leak sit without an owner.** If the homepage or a paid landing page is the largest leak by volume and has no Command Center item, add one. This is the gap the OS used to miss: it triaged steps inside the funnel and never questioned the page.

### 2c. Coverage scan — close the blind spots the OS was not told to track
This is the generalized version of the per-experiment "coverage check": build.py's **Coverage Reconciler** runs every build, diffs the live marketing surface (events / experiments / open ClickUp tasks) against `coverage-registry.json` (what the OS is told to model), and writes the gaps to `live.coverage`. It is detect-and-escalate only — it never wires anything itself and never touches `content.json`. Resolving each escalated item is **your** job, and it is the whole loop in v1 (nothing is auto-wired).
- **Read `live.coverage`** (also shown in the Coverage tab + the footer health pill). `blindSpots` = live surface the OS does not model, ranked by volume; `stale` = a modeled event that stopped firing (possible tracking regression); `autowired` is empty in v1.
- **For each escalated blind spot, investigate, then either wire it in or dismiss it — never leave it sitting.**
  - *Event cluster* (e.g. the `tbs_*` "try before signup" flow): query the events to map the real sub-funnel (the `tbs_*` playbook), then wire it into the OS — add a funnel stage in `content.json` and/or a Command Center item — **or** dismiss it to `coverage-registry.json` `dimensions.events.ignore` with a one-line reason in `ignoreReasons`.
  - *Experiment-looking ClickUp task* (weak name match): confirm it is a real, instrumented experiment. If yes, wire an `OS-…` experiment card in `content.json` (and add the task id to `experiments.clickupMapped`); if not, dismiss it to `experiments.ignore` with a reason. **A name match is not a green light to auto-stub a card** — the regex is judgment-only, so a false match must be dismissed, not materialized into the user-facing Experiments tab.
  - *Unsurfaced marketing ClickUp task*: either give it a Command Center item or add it to `clickupTasks.ignore` (with a reason) / `clickupTasks.modeled` if a card already covers it.
- **A `stale` entry means a modeled event went quiet.** Decide whether it is a real tracking regression (file a fix + Command Center item) or simply has no volume yet (e.g. `purchase_completed` with no sales) — in the latter case leave it; it self-clears when the event fires.
- **You own `content.json`.** Any funnel stage, experiment card, or Command Center item lands here, in this reasoning lane — that is where unknown → known completes. The build only ever writes `data.js` (the `live.coverage` block) and `coverage-registry.json` (your dismissals). Turning on a flow or launching a live test still escalates to Alejo (see the safety boundary).
